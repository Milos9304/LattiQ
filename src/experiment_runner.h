...
class AlphaMinimizationExperiment{
	...
	inline double strategy_random_alpha_c(std::vector<std::vector<AlphaMinimizationExperimentInstance>> train_dataset, std::vector<double> angles, std::string meta_data, std::string* optimized_by, int iteration_i, int *p_num, int **p_inst);
	...
	double _cost_fn(std::vector<AlphaMinimizationExperimentInstance>, const double *angles, std::string meta_data, int probability100=100, int* p_inst=nullptr);
	FastVQA::QAOAOptions* qaoaOptions;
	MapOptions* mapOptions;
	Database* database;
};
