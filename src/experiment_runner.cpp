...
void AngleResultsExperiment::run_qaoa_with_optimizer(){
	...
	std::cerr<<std::endl<<python_output<<std::endl;
	...
	for(auto &instance: (*dataset)){
		...
		if(use_database){throw;
			...
			if(meta_data != "optCMQAOA" && meta_data != "optQAOAnonpen"){
				...
				if(instance.h.nbQubits == 14 && plot_histogram){

					long double *real = buffer.stateVector->stateVec.real;
					long double *imag = buffer.stateVector->stateVec.imag;

				    int arraySize = pow(2,14);//sizeof(real) / sizeof(real[0]);
				    //std::cerr<<arraySize;

					std::vector<long double> realV(arraySize);
					std::vector<long double> imagV(arraySize);

					std::copy(real, real + arraySize, realV.begin());
					std::copy(imag, imag + arraySize, imagV.begin());

					std::vector<int> V(arraySize);
					std::iota(V.begin(),V.end(),0); //Initializing

					Eigen::Matrix<qreal, Eigen::Dynamic, Eigen::Dynamic> M = instance.h.getMatrixRepresentation2(true);

					std::sort(V.begin(),V.end(), [&](int i,int j){return M(i,i)<M(j,j);});

					//std::cerr<<instance.zero_solutions[0].index<<" "<<instance.sv_solutions[0].index<<std::endl;
					//std::cerr<<V[0]<<" "<<V[1]<<std::endl;throw;

					for(unsigned long i = 0; i < pow(2,14); ++i){
						histogram[i] += real[V[i]]*real[V[i]]+imag[V[i]]*imag[V[i]];
						//std::cerr<<"."<<" "<<v<<" ";
					}//std::cerr<<std::endl;
		...
		if(this->evalOutput){
			...
			*/
			...
			for(long long int j = 1; j < buffer.stateVector->numAmpsTotal; ++j){
					...
					long long int index = refEnergies[j].index;
					if(refEnergies[j].value <= refEnergies[1].value * pow(instance.h.nbQubits, 4)){
...
\
				...
				for(int i_rand_angles = 0; i_rand_angles < max_i_rand_angles; i_rand_angles++){
					...
					std::cerr<<nlopt_res_to_str(result.second)<<std::endl;
...
double AlphaMinimizationExperiment::_cost_fn(std::vector<AlphaMinimizationExperimentInstance> dataset, const double *angles, std::string meta_data, int probability100, int* p_inst){
	std::vector<int> num_sols;
